...
async function main() {
  ...
  // ==================== CATEGORIES ====================
  // Root categories
  const continentCategory = await prisma.category.create({
    data: {
      name: "Continent",
    },
  });

  const genderCategory = await prisma.category.create({
    data: {
      name: "Gender",
    },
  });

  const roleCategory = await prisma.category.create({
    data: {
  ...
  const africaCategory = await prisma.category.create({
    data: {
      name: "Africa",
  ...
  const europeCategory = await prisma.category.create({
    data: {
      name: "Europe",
      parentId: continentCategory.id,
    },
  ...
  const americasCategory = await prisma.category.create({
    data: {
      name: "Americas",
      parentId: continentCategory.id,
    },
  ...
  const maleCategory = await prisma.category.create({
    data: {
      name: "Male",
  ...
  const femaleCategory = await prisma.category.create({
    data: {
      name: "Female",
      parentId: genderCategory.id,
    },
  ...
  // Role subcategories
  const scientistCategory = await prisma.category.create({
    data: {
      name: "Scientist",
  ...
  const musicianCategory = await prisma.category.create({
    data: {
      name: "Musician",
      parentId: roleCategory.id,
    },
  ...
  const athleteCategory = await prisma.category.create({
    data: {
      name: "Athlete",
      parentId: roleCategory.id,
    },
  ...
  const revolutionaryCategory = await prisma.category.create({
    data: {
      name: "Revolutionary",
      parentId: roleCategory.id,
    },
  ...
  // ==================== FIGURES ====================
  const figures = await prisma.figure.createMany({
    data: [
      {
        name: "Nelson Mandela",
        title:
          "Anti-apartheid activist and first Black President of South Africa",
  ...
  await prisma.figureCategory.createMany({
    data: [
      {
        figureId:
          figures.count > 0 ? (await prisma.figure.findFirst())?.id || "" : "",
        categoryId: africaCategory.id,
      },
      {
        figureId:
          figures.count > 1
            ? (await prisma.figure.findMany({ take: 2 }))[1]?.id || ""
            : "",
  ...
  const path2 = await prisma.learningPath.create({
    data: {
      ...
      description:
        "Discover the contributions of women scientists throughout history",
      difficulty: 3,
      isPublished: true,
    },
  });
  ...
  await prisma.lessonFigure.createMany({
    data: [
      {
        lessonId: lesson1.id,
        figureId:
          (await prisma.figure.findFirst({ where: { name: "Nelson Mandela" } }))
            ?.id || "",
      },
      {
        lessonId: lesson2.id,
        figureId:
          (await prisma.figure.findFirst({ where: { name: "Marie Curie" } }))
            ?.id || "",
      },
    ],
    skipDuplicates: true,
  ...
  // ==================== WEEKLY REWARDS ====================
  await prisma.weeklyReward.createMany({
    data: [
